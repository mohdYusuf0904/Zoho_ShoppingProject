...
public class FashionStore {
	...
	private void loadFlipFLop() {
		...
		for(int i=0;i<3;i++)
		{
			flipflop = new Fashion.FashionBuilder(person[i] +" Flipflop",480)
					   .setBrand(brand[i])
					   .setColour(color[i%2])
					   .setDescription(person[i] +" " +color[i%2] +" Typography Print Sliders")
					   .setPerson(person[i])
					   .setSize(size[i])
					   .setCatagory("Casual")
					   .getNewFashion();
			...
			flipflop = new Fashion.FashionBuilder(person[i] +" Flipflop",1180)
					   .setBrand(brand[i])
					   .setColour(color[i%2])
	...
	private void loadSandals() {
		...
		for(int i=0;i<3;i++)
		{
			sandals = new Fashion.FashionBuilder(person[i] +" Sandals",1180)
					   .setBrand(brand[i])
					   .setColour(color[i%2])
					   .setDescription(person[i] +" " +color[i%2] +" Relay MU Sports Sandals")
					   .setPerson(person[i])
					   .setSize(size[i])
					   .setCatagory("Casual")
					   .getNewFashion();	
			...
			sandals = new Fashion.FashionBuilder(person[i] +" Sandals",780)
					   .setBrand(brand[i])
					   .setColour(color[i%2])
	...
	private void loadShoe() {
		...
		for(int i=0;i<3;i++)
		{
			shoe = new Fashion.FashionBuilder(person[i] +" Shoe",2180)
					   .setBrand(brand[i])
					   .setColour(color[i%2])
					   .setDescription(person[i] +" " +color[i%2] +" Mesh Running Shoes")
					   .setPerson(person[i])
					   .setSize(size[i])
					   .setCatagory("Casual")
					   .getNewFashion();	
			...
			shoe = new Fashion.FashionBuilder(person[i] +" Shoe",2180)
					   .setBrand(brand[i])
					   .setColour(color[i%2])
					   .setDescription(person[i] +" " +color[i%2] +" Semiformal Shoes")
					   .setPerson(person[i])
					   .setSize(size[i])
					   .setCatagory("Formal")
					   .getNewFashion();	
	...
	private void loadTshirt() {
		...
		for(int i=0;i<3;i++)
		{
			tshirt = new Fashion.FashionBuilder(person[i]+" Tshirt", 350)
				.setPerson(person[i])
				.setDescription( "Dnmx " + s[i%2] + " "+ colour[i%2] + "Color Block " + person[i]+" Round Neck T-Shirt")
				.setCatagory("Roundneck")
				.setSize(s[i%2])
			    .setColour(colour[i%2])
			    .setBrand("Dnmx").getNewFashion();
			
			ProductManager.getInstance().addProducts(tshirt);

			tshirt = new Fashion.FashionBuilder(person[i]+" Tshirt", 440)
			.setPerson(person[i])
			.setDescription("Dnmx " + s[i%2] + " "+ colour[i%2] + "Color Block " + person[i]+" Colar Neck T-Shirt")
			.setCatagory("Colar neck")
			.setSize(s[i%2])
			.setColour(colour[i%2])
			.setBrand("Dnmx").getNewFashion();
			ProductManager.getInstance().addProducts(tshirt);
	...
	private void loadWinterWear() {
		...
		for(int i=0;i<3;i++)
		{
			...
			winterWear = new Fashion.FashionBuilder("Winterwear "+person[i]+" Hoodie",950)
			.setPerson(person[i])
			.setSize(s[i%2])
			.setColour(colour[i%2])
			.setDescription(brand[i] + s[i%2]+ " "+ colour[i%2] +  " Slim Fit Hoodie with Kangaroo Pocket ")
			.setBrand(brand[i])
			.setCatagory("Hoodie").getNewFashion();
			ProductManager.getInstance().addProducts(winterWear);
			
			winterWear = new Fashion.FashionBuilder("Winterwear " +person[i]+" Hoodie",850)
			.setPerson(person[i])
			.setSize(s[i%2])
			.setColour(colour[i%2])
			...
			.setBrand(brand[i])
			.setCatagory("Sweatshirt").getNewFashion();
			ProductManager.getInstance().addProducts(winterWear);
	...
	private void loadSarees() {
		...
		String description[]= {
				...
				"Embroidered Fashion Lycra Saree ",
				"Printed Mysore Art Saree",
		...
		for(int i=0;i<3;i++)
		{
			...
			saree = new Fashion.FashionBuilder("Saree",820)
			.setPerson("Women")
			.setDescription(description[i])
			.setColour(colour[i%2])
			.setSize("L")
			.setBrand(brand[i])
			.setCatagory(catagory[i]).getNewFashion();
			ProductManager.getInstance().addProducts(saree);
			
			
			saree = new Fashion.FashionBuilder("Saree",520)
					.setPerson("Women")
					.setDescription(description[i])
					.setColour(colour[i%2])
					.setSize("L")
					...
					saree = new Fashion.FashionBuilder("Saree",780)
					.setPerson("Women")
					.setDescription(description[i])
					.setColour(colour[i%2])
					.setSize("L")
